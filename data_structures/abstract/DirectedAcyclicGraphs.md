---
layout: cd1
title: DirectedAcyclicGraphs
---

# DirectedAcyclicGraphs

<div>
<strong>Author:</strong> Scott Morgan<br/>
<strong>Created:</strong> 2025-12-12<br/>
<strong>Edited:</strong> 2025-12-12<br/>
<strong>Id:</strong> 1.3.6.1.4.1.33097.1.0.16<br/>
<strong>Copywrite 2025 Adligo Inc</strong>
</div>
<br/>

In mathematics, particularly graph theory, and computer science, a directed acyclic graph (DAG) is a directed [Graph#1.3.6.1.4.1.33097.1.0.14](Graphs.md) with no directed [Cycles#1.3.6.1.4.1.33097.1.0.12.5](NodeNetworks.md#cycles).  DirectedAcyclicGraphs may take the form of a single [NodeNetwork#1.3.6.1.4.1.33097.1.0.12](NodeNetworks.md) or [NodeNetworkSet#1.3.6.1.4.1.33097.1.0.15](NodeNetworkSets.md) with more than one [NodeNetwork#1.3.6.1.4.1.33097.1.0.12](NodeNetworks.md).


##### Questions Comments:
- [papers.adligo.com/issues](https://github.com/adligo/papers.adligo.com/issues)

- [Papers Index](../README.md)

### Citations 
[https://en.wikipedia.org/wiki/Graph_theory](https://en.wikipedia.org/wiki/Graph_theory)

[https://en.wikipedia.org/wiki/Directed_acyclic_graph](https://en.wikipedia.org/wiki/Directed_acyclic_graph)

[https://arxiv.org/pdf/2308.04512](https://arxiv.org/pdf/2308.04512)