# Sets

<div><strong>
Author: Scott Morgan<br/>
Edited: 2025-11-25<br/>
Id: 1.3.6.1.4.1.33097.1.0.6<br/>
</strong></div>

A set derived from the mathematical term, is simply a collection of different things.

### Citations

[https://en.wikipedia.org/wiki/Set_(mathematics)](https://en.wikipedia.org/wiki/Set_(mathematics))

[https://www.geeksforgeeks.org/maths/set-notation/](https://www.geeksforgeeks.org/maths/set-notation/)

[https://docs.oracle.com/javase/8/docs/api/java/util/Set.html](https://docs.oracle.com/javase/8/docs/api/java/util/Set.html)