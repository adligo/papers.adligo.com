---
layout: cd1
title: BinarySearch
---

# BinarySearch

<div>
<strong>Author:</strong> Scott Morgan<br/>
<strong>Created:</strong> 2025-11-25<br/>
<strong>Edited:</strong> 2025-12-13<br/>
<strong>Id:</strong> 1.3.6.1.4.1.33097.0.2.1<br/>
<strong>Copywrite 2025 Adligo Inc</strong>
</div>

BinarySearch is a searching algorithm that operates on a sorted or monotonic search space, repeatedly dividing it into halves to find a target value or optimal answer in logarithmic time O(log N).

### Notes

Was reidentified from 1.3.6.1.4.1.33097.0.0 to 1.3.6.1.4.1.33097.0.2.1
on 2025-12-13.

##### Questions Comments:
[papers.adligo.com/issues](https://github.com/adligo/papers.adligo.com/issues)

[Papers Index](../../README.md)

### Citations 

- [https://www.geeksforgeeks.org/dsa/binary-search/](https://www.geeksforgeeks.org/dsa/binary-search/)

- [https://en.wikipedia.org/wiki/Binary_search](https://en.wikipedia.org/wiki/Binary_search)
- [https://leetcode.com/problems/binary-search/description/](https://leetcode.com/problems/binary-search/description/)
- [https://www.khanacademy.org/computing/computer-science/algorithms/binary-search/a/binary-search](https://www.khanacademy.org/computing/computer-science/algorithms/binary-search/a/binary-search)